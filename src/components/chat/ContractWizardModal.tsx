...
export default function ContractWizardModal({
  ...
  const canSubmit =
    (isReview ? clientSig !== "" && professionalSig !== "" : mySig !== "") &&
    verified;
  ...
  const articles = [
    {
      title: "제 1 조 (용역의 내용)",
      body:
        serviceContent || "본 계약에서 수행하고자 하는 용역의 내용을 명시한다.",
    },
    {
      title: "제 2 조 (용역의 기간)",
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-10 py-8 border-r border-zinc-200 bg-zinc-50/60">
        <div className="relative isolate max-w-2xl mx-auto bg-white rounded-xl shadow-sm ring-1 ring-zinc-100 px-10 py-10 flex flex-col gap-6">
          ...
          <h1 className="relative z-10 text-2xl font-bold text-contract text-center tracking-widest">
            용 역 계 약 서
          </h1>
          <p className="relative z-10 text-sm text-zinc-700 leading-relaxed">
            <strong className="text-contract">{clientName || " "}</strong>
            {josa(clientName, "과", "와")}(이하 &apos;갑&apos;이라 한다){" "}
            <strong className="text-contract">{professionalName || " "}</strong>
            {josa(professionalName, "은", "는")}(이하 &apos;을&apos;이라 한다)
            아래 용역에 관하여 다음과 같이 계약을 체결한다.
          </p>
          <div className="relative z-10 flex flex-col gap-3">
            {articles.map((article) => (
              <div key={article.title} className="flex flex-col gap-1">
                <span className="font-bold text-contract text-xs">
                  {article.title}
                </span>
      ...
      <div className="w-80 shrink-0 overflow-y-auto px-6 py-8 flex flex-col gap-5">
        <div className="flex items-center gap-2 text-sm">
          <span
            className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${step === 1 ? "bg-main text-white shadow-sm shadow-main/30" : "bg-zinc-100 text-zinc-400"}`}
          >
            {step === 2 ? <IoCheckmark className="text-sm" /> : 1}
          </span>
          <span
            className={`transition-colors ${step === 1 ? "text-main font-semibold" : "text-zinc-400"}`}
          >
            기본 정보 입력
          </span>
          <div
            className={`flex-1 h-px transition-colors ${step === 2 ? "bg-main" : "bg-zinc-200"}`}
          />
          <span
            className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${step === 2 ? "bg-main text-white shadow-sm shadow-main/30" : "bg-zinc-100 text-zinc-400"}`}
          >
            2
          </span>
          <span
            className={`transition-colors ${step === 2 ? "text-main font-semibold" : "text-zinc-400"}`}
          >
            서명
          </span>
        </div>
        ...
        {step === 1 ? (
          ...
          <div className="flex flex-col gap-4">
            {!verified ? (
              ...
              <SignaturePad
                label="을"
                onSave={setProfessionalSig}
                savedUrl={professionalSig}
              />
            )}
