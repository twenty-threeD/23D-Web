...
async function fetchGrayImageBytes(src: string): Promise<ArrayBuffer | null> {
  const imageBytes = await fetchImageBytes(src);
  if (!imageBytes) return null;

  try {
    const image = await createImageBitmap(new Blob([imageBytes]));
...
export async function buildContractPdf(data: ContractPdfData): Promise<Blob> {
  ...
  const watermarkBytes = await fetchGrayImageBytes(WATERMARK_URL);
  const watermark = watermarkBytes ? await doc.embedPng(watermarkBytes) : null;
  ...
  function drawWatermark() {
    ...
    page.drawImage(watermark, {
      x: PAGE_WIDTH / 2 - rotatedWidth / 2,
      y: PAGE_HEIGHT / 2 - rotatedHeight / 2,
      width,
      height,
      opacity: 0.09,
      rotate: degrees(45),
